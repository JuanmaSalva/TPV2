...
void FightersSystem::recieve(const msg::Message& msg)
{
	...
	case msg::_FIGHTER_INFO: {
		...
		if (game_->getNetworking()->getClientId() != m.senderClientId) { //mensaje del otro caza
			if (game_->getNetworking()->getClientId() == 1) {
				Transform* tr = fighter0_->getComponent<Transform>(ecs::Transform);
				tr->position_ = Vector2D(m.posX, m.posY);
				tr->rotation_ = m.rot;
			}
			else {
				Transform* tr = fighter1_->getComponent<Transform>(ecs::Transform);
				tr->position_ = Vector2D(m.posX, m.posY);
				tr->rotation_ = m.rot;
			}
		}
